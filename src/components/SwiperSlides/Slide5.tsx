...
const TopArtist = ({ isActive }: { isActive: boolean }) => {
  ...
  return (
    <div className="flex flex-col items-center md:items-start md:flex-row w-full gap-4 px-4 md:px-8 pb-8 sm:pt-16">
      <h1
        className={`text-white block md:hidden text-2xl pb-4 text-start md:text-[35px] font-bold leading-8 max-w-[320px] transition duration-1000 delay-500 ease-linear ${
      ...
      <div className="flex-1 flex justify-start">
        <Tilt glareEnable={true} glareMaxOpacity={0.1} scale={1.1}>
              ...
              <CardContent className="pb-4">
                ...
                {topArtist.percentage !== undefined && (
                  <p className="text-zinc-400 text-sm text-center pt-4 font-medium">
                    You have spent {Math.round(topArtist.percentage)}% of your
                    listening time with {topArtist.name}.
                  </p>
                )}
              </CardContent>
            </Card>
        ...
        <div>
          <h1
            className={`text-white text-[35px] font-bold leading-10 max-w-[380px] transition-transform duration-700 ${
              isActive ? "fade-left-up-animation" : ""
            }`}
          >
            Listened to multiple artists, but one truly stole your heart...
          </h1>
          <p
            className={`text-zinc-400 font-normal max-w-[380px] leading-6 mt-14 text-lg transition duration-700 ${
      ...
      <p
        className={`text-zinc-400 block md:hidden font-normal text-start max-w-[320px] leading-6 pt-4 transition duration-700 ${
          showText ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"
        }`}
      >
        Your most listened track by
